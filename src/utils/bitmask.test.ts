...
describe("Permission Bitmask Utilities", () => {
  describe("createBitmask", () => {
    it("should create correct bitmask", () => {
      expect(createBitmask({ group: TestPermissionGroup.USER, read: false, write: false, delete: false, update: false })).toBe(
        bitmask0
      );
      expect(createBitmask({ group: TestPermissionGroup.POST, read: true, write: true, delete: true, update: true })).toBe(bitmask1);
      expect(createBitmask({ group: TestPermissionGroup.COMMENT, read: true, write: false, delete: false, update: false })).toBe(
        bitmask2
      );
      expect(createBitmask({ group: TestPermissionGroup.LIKE, read: true, write: true, delete: true, update: true })).toBe(bitmask3);
      expect(createBitmask({ group: TestPermissionGroup.POST, read: true, write: false, delete: false, update: true })).toBe(
        bitmaskWithUpdate
      );
    });
  });
  ...
  describe("hasPermissionAccess", () => {
    it("should correctly check access presence", () => {
      ...
      expect(hasPermissionAccess(bitmaskWithUpdate, PermissionAccess.WRITE)).toBe(false);
    });
  });
  ...
  describe("access checking utilities", () => {
    ...
    it("should verify write access", () => {
      ...
      expect(canWrite(bitmaskWithUpdate)).toBe(false);
    });
